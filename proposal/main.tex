...
% \section{Formal Design}
% In this section, we discuss the formal design a relatively high level. Using the library-provided
% macros, the developer

% \begin{enumerate}
  % \item defines a set of agents \(I^{*} = \{a_1, a_2, \ldots\}\), such that the set of all agents
    % \(I = I^{*} \cup \{a_0\}\), where $a_0$ denotes a default agent.

  % \item declares a set of epistemic obligations $\Phi$ in relation to certain data. These
    % obligations are statements in the language of \cref{sec:background-del}.

  % \item erases certain announcements by declaring the independence of the associated data from
    % otherwise dependent data.
% \end{enumerate}
